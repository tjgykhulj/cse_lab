...
{
	//to imitate the fuseserver_readdir
	yfs_client::inum inum = ino; // req->in.h.nodeid;
...
{
	//copy and modify from fuseserver_ mkdir
    struct fuse_entry_param e;
    // In yfs, timeouts are always set to 0.0, and generations are always set to 0
    e.attr_timeout = 0.0;
    e.entry_timeout = 0.0;
	...
	yfs_client::inum inum;
    fprintf(fp, "y %lld %s %s\n",parent,name,link);
	fflush(fp);
	yfs_client::status ret = yfs->symlink(parent, name, link, inum);
	if (ret != yfs_client::OK) goto release;
	e.ino = inum;
	ret = getattr(inum, e.attr);
...
void clean(yfs_client::inum inum)
{
	list<yfs_client::dirent> all;
	yfs->readdir(inum, all);
	for(list<yfs_client::dirent>::iterator i=all.begin(); i!=all.end(); i++)
	{
		clean(i->inum);
		yfs->unlink(inum, i->name.c_str());
	}
}
void work(int version) 
{
	clean(1);
	FILE *f2 = fopen("tjg.log","r");
	char c[3];
	yfs_client::inum parent, ino;
	char buf[200000], buf2[1024];
    int size, off;
	for (int i=0; i<version; i++)
	while (fscanf(f2,"%s",c)) 
		if (c[0] == 's') {
			fscanf(f2, "%lld %d", &ino, &size);
			yfs->setattr(ino, size);
		} else
		if (c[0] == 'c') {
			fscanf(f2, "%lld %s", &parent, buf);
			yfs->create(parent,buf,0,ino);
		} else
		if (c[0] == 'm') {
			fscanf(f2, "%lld %s", &parent, buf);
			yfs->mkdir(parent,buf,0,ino);
		} else
		if (c[0] == 'y') {
			fscanf(f2, "%lld %s %s", &parent, buf, buf2);
			yfs->symlink(parent,buf,buf2,ino);
		} else
		if (c[0] == 'w') {
			fscanf(f2, "%lld %d %d", &ino, &size, &off);
			fgets(buf,2,f2);
			fgets(buf,200000,f2);
			buf[strlen(buf)-1]=0;
			size_t temp;
			yfs->write(ino,size,off,buf,temp);
		} else
		if (c[0] == 'u') {
			fscanf(f2, "%lld %s", &parent, buf);
			yfs->unlink(parent, buf);
		} else 
		if (c[0] == 'v') {
			break;
		}
